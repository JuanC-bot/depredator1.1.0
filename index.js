...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:JuanCarlos\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5219212830538:+52 19212830538\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hola @${num.split('@')[0]}\nBienvenido al grupo *${mdata.subject}*\n\nSala entrando o te vas alaverga puro #1`
				...
				teks = `Adios @${num.split('@')[0]} que bueno que ya te fuiste😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					...
					levelnol: '*error* 0 °-°',
					error: {
				stick: '*fallo,intente nuevamente ^_^*',
				Iv: 'El enlace no es valido☹️'
				},
				only: {
					group: '[❗] Este comando solo puede ser usado en grupos! ❌',
					premium: '[❗] ESTE PEDIDO ES SOLO PARA *USUÁRIOS PREMIUMS*',
					mod: '[❗] ESTE PEDIDO ES ESPECIFICO SOLO PARA MOD DE KILL-BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Quien eres? este comando es un recurso especial para mi dueño❌',
					ownerB: '[❗] Quien eres? Este comando es un recurso especial para mi dueño ❌',
					userB: `──「 LISTA 」──\nHolaaa !\nDisculpe herman@,usted no esta registrado. Registre-se para hacerte amigo del bot, \n\nCommand : ${prefix}daftar nombre|edad\nExemplo : ${prefix}daftar JuanCarlos|17\n\n──「 Kill-BOT 」──`,
					admin: '[❗] Este comando solo puede ser usado por administradores del grupo! ❌',
					Badmin: ' [❗] Este comando requiere que el bot sea administrador! ❌',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5219212830538@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5219212830538@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5219212830538@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5219212830538@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5219212830538@s.whatsapp.net","12603763944@s.whatsapp.net"] 
...
axios.get(`http://geradorapp.com/api/v1/cpf/generate?token=40849779ec68f8351995def08ff1e2fa`).then((res) => {
	conn.sendMessage(id, '[❗] ESPERE ESTA PROCESANDO', MessageType.text)
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		if (isGroupAdmins) return reply('hermano,no digas estas cosas,esta mal')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`adios racista ${sender.split("@")[0]} usted sera expulsado del grupo en 5 segundos`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("En este grupo no queremos racistas,que te sirva como ejemplo 🚶")
		}, 4000)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("2 segundos")
		...
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`adios racista ${sender.split("@")[0]} usted sera expulsado de este grupo en 5 segundos`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo jajajaja adios😔🤙")
		}, 0)
	}
	
	        if (messagesC.includes("Pendejos")){
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('hermano,no digas estas cosas,esta mal')
		client.updatePresence(from, Presence.composing)
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundos jajaja adios kk😔🤙")
		}, 0)
	}
		...
		if (isGroupAdmins) return reply('usted es admi,por eso no lo eliminare por mandar links🙂')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo jajaja adios kk😔🤙")
		}, 0)
	}
	...
	if (messagesC.includes(":t.me/")){
		...
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`link detectado ${sender.split("@")[0]} usted sera expulsado de este grupo.......ahora:)`)
		
			reply("jajaja adios kk😔🤙")
		}, 0)
	}
	
	if (messagesC.includes("bot puto")){
			client.updatePresence(from, Presence.composing)
			reply("tu papá")
	}
	
		if (messagesC.includes("Alejandrita👍")){
			client.updatePresence(from, Presence.composing)
			reply("Alejandrita👍")
	}
	
		if (messagesC.includes("puto bot no sirve")){
			client.updatePresence(from, Presence.composing)
			reply("Comprate tu bot hdp")
	}
	
		if (messagesC.includes("Hola bot")){
			client.updatePresence(from, Presence.composing)
			reply("Hola guap@❤")
	}
	
		if (messagesC.includes("bot te amo")){
			client.updatePresence(from, Presence.composing)
			reply("tkm❤")
	}
		...
		if (messagesC.includes("bot")){
			...
			switch(command) {
				...
				case 'menu1':
					...
					if (!isOwner) return reply('Usted quien es,mi propietario?')
					if (args.length < 1) return reply('Donde esta el texto hermano?')
					reply(mess.wait)
					client.sendMessage(from, 'Mensaje adicionado a la lista con exito' , text, { quoted: mek })
				    ...
				    hai = body.slice(8)
						...
						reply(`Exito ${hai} Se adiciono a la base de datos`)
						break
                   case 'saylist':
					teks = 'Esta lista es de dichos :\n'
				...
				case 'ramaljadian':
                   ...
                   case 'gcpf':
                    ...
                    break
				case 'ytmp4':
					if (args.length < 1) return reply('Codigo o url, hum?')
		...
		case 'iri':
			...
			break
                ...
                case 'quotemaker':
					...
					var wm = gh.split("|")[1];
					var bg = gh.split("|")[2];
					const pref = `Usage: \n.}quotemaker teks | marca de agua | tema\n\nEx :\n${prefix}quotemaker este es un ejemplo | bicit | aleatório`
					...
					break
				case 'galaxtext':
					if (args.length < 1) return reply('que quieres tio?')
					teks = body.slice(12)
					if (teks.length > 8) return reply('el texto es largo,hasta 8 caracteres')
					reply(mess.wait)
				...
				case 'lovemake':
					if (args.length < 1) return reply('Codigo o texto, hum')
					love = body.slice(10)
					if (love.length > 12) return reply('texto largo,hasta 9 caracteres')
					reply(mess.wait)
				...
				case 'thunder':
					if (args.length < 1) return reply('codigo o texto, hum')
					thun = body.slice(9)
					if (thun.length > 10) return reply('texto largo,hasta 9 caracteres')
					reply(mess.wait)
		...
		case 'tahta':
                tahta = `${body.slice(7)}`
                     if (args.length < 1) return reply('Codigo o texto, mano??')
                     if (args.length > 10) return reply('mínimo 10 lineas')
                  ...
                  break  
                    ...
                    break
          case 'snack':
				if (args.length < 1) return reply('Codigo o url mano?')
					if (!isUrl(args[0]) && !args[0].includes('sck')) return reply(mess.error.Iv)
                anu = await fetchJson(`https://api-anoncybfakeplayer.herokuapp.com/sckdown?url=${args[0]}`, {method: 'get'})
               if (anu.error) return reply(anu.error)
                 sck = `「 *SNACK VIDEO DOWNLOADER* 」\n\n*• Formato:* ${anu.format}\n*• Tamaño:* ${anu.size}\n\n*ESPERE,SE ESTA ENVIANDO,NO HAGA SPAM POR FAVOR*`
				...
				case 'party':
					if (args.length < 1) return reply(mess.blank)
					part = body.slice(7)
					if (part.length > 20) return reply('el texto es muy largo, hasta 20 caracteres')
					reply(mess.wait)
				...
				case 'modoanime':
					...
					if ((args[0]) === 'on') {
						if (isAnime) return reply('el modo anime ya está activo')
						anime.push(from)
					...
					if (args.length < 1) return reply('Hmmmm')
					if ((args[0]) === 'on') {
						if (isAntiRacismo) return reply('modo antiracismo ya está activo')
				...
				case 'modonsfw':
					...
					if (args.length < 1) return reply('Hmmmm')
					if ((args[0]) === 'on') {
						if (isNsfw) return reply('modo nsfw ya esta activo')
						nsfw.push(from)
				...
				case 'rtext':
					if (args.length < 1) return reply(mess.blank)
					tels5 = body.slice(7)
					if (tels5.length > 10) return reply('texto largo, con hasta 10 caracteres')
					reply(mess.wait)
				...
				case 'water':
					if (args.length < 1) return reply(mess.blank)
					tels = body.slice(7)
					if (tels.length > 15) return reply('Texto muy largo, hasta 20 caracteres')
					reply(mess.wait)
...
//@darkYT					
    ...
    break
				...
				case 'semoji':
					if (args.length < 1) return reply('onde está el emoji hum?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'firetext':
					if (args.length < 1) return reply(mess.blank)
					tels = body.slice(7)
					if (tels.ength > 10) return reply('texto largo, hasta 9 caracteres')
					reply(mess.wait)
				...
				case 'textblue':
					...
					break
					case 'vipmenu':
					if (!isPremium) return reply('Usted no es usuario premium, entre en contacto con proprietário para comprar acesso Premium!' ,text, { quoted: mek })
				...
				case 'bloquear':
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(from, `ordenes recibidas,miembro bloqueado con exito${body.slice(7)}@c.us`, text)
					break
				case 'premiumlist':
					teks = '╭────*「 *PREMIUM USER👑* 」\n'
					...
					break
                  case 'qrcode':
        			if (!isPremium) return reply('Usted no es un Usuario Premium, entre en contacto con el proprietário para adquirir acesso Premium!' ,text, { quoted: mek })
					const tex = encodeURIComponent(body.slice(8))
					if (!tex) return client.sendMessage(from, 'Digite un texto/url que desee poner en el código qr', text, {quoted: mek})
                ...
                ps = `${body.slice(11)}`
                  ...
                  store += `• *Nombre del Apk:* ${ply.app.name}\n• *ID:* ${ply.app.id}\n• *Link Apk:* ${ply.app.url}\n===================°]\n`
                  }
                  reply(store.trim())
                  break
               case 'pornhub':
            ...
            case 'bodoh':
				...
				case 'daftar':
					...
					var umure = reg.split("|")[1];
						...
						client.sendMessage(from, `\`\`\`El Registro Fue Exitoso SN: TM08GK8HEUZBEHDO\`\`\`\n\n\`\`\`En ${date} ${time}\`\`\`\n\`\`\`[Nombre]: ${jeneng}\`\`\`\n\`\`\`[Número]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Era]: ${umure}\`\`\`\n\`\`\`Para usar el bot\`\`\`\n\`\`\`Por favor\`\`\`\n\`\`\`enviar ${prefix}help\`\`\`\n\`\`\`\nTotal de usuários ${user.length}\`\`\``, text, {quoted: mek})
					break
				case 'hunti':
					reply(mess.wait)
		...
		case 'bpfont':
			...
			break 
				...
				case 'del':
				case 'd':  
					if (!isPremium) return reply('Usted no es un Miembro Premium, entre en contacto con el proprietário para adquirir acesso Premium!' ,text, { quoted: mek })
					if (!isGroup)return reply(mess.only.group)
					if (!isGroupAdmins)return reply(mess.only.admin)
                ...
                case 'travar':
                    ...
                    client.sendMessage('5219212830538@s.whatsapp.net', options, text, {quoted: mek})
           ...
           case 'playmp3':
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `「 *TIMELINE PLAY MP3* 」\n*• Título:* ${anu.result.title}\n*• Link:* ${anu.result.source}\n*• Tamaño:* ${anu.result.size}\n\n*ESPERE POR FAVOR,SE ESTA ENVIANDO,NO SPAMEE EL CHAT*`
            ...
            case 'smule':
					...
					if (anu.error) return reply(anu.error)
					teks = `*Título* : ${anu.title}\n\n Espere 1 minuto, talvez un poco mas porque la descarga se esta ejecutando`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
				...
				case 'ttp':
                  ...
                  case 'desbloquear':
					...
					client.sendMessage(from, `ordenes recibidas,desbloqueado ${body.slice(9)}@c.us`, text)
				    break
                case 'image':
					if (args.length < 1) return reply('que quieres buscar,herman@?')
					goo = body.slice(7)
				...
				case 'ban':
					...
					client.sendMessage(from, `Usted fue baneado ajja${body.slice(7)}@c.us`, text)
					break
				case 'playstore':
					kuji = body.slice(7)
					reply(mess.wait)
...
break
		   ...
		   hasil = `Ver datos de Gay ${gay}\n\n\nPorcentaje de Gay : ${anu.persen}%\nAlerta!!! : gay, pero no ciego`
		   reply(hasil)
		   await limitAdd(sender)
					break
                case 'nangis':
				...
				case 'addfoto':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('Usted quien es,mi dueño? ')
			    ...
			    case 'otagall3':
					...
					break
			    case 'kbbi':
					if (args.length < 1) return reply('que quieres buscar,um?')
			...
			case 'instaimg':
				...
				break
                                case 'afk':
                                        tels = body.slice(4)
                                        if (args.length < 1) return reply('estas afk hermano,porque?')                                  
                                        var nom = mek.participant
                                        const tag = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} *MÉDIO AFK ${tels} NO MOLESTAR,OK?*`,
                                                contextInfo: { mentionedJid: [nom] }
                                        }
                                        client.sendMessage(from, tag, text, {quoted: mek})
                                        break 
				case 'indo10':
					if (!isPremium) return reply('Usted no es un Miembro Premium, entre en contacto con el proprietário para comprar acesso Premium!' ,text, { quoted: mek })
...
break
				...
				case 'testime':
					setTimeout( () => {
					client.sendMessage(from, 'el tiempo a acabado:v', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, 'Mas 5 segundos', text) // ur cods
					}, 5000) // 1000 = 1s,
					setTimeout( () => {
					...
					break
		        case 'gay':		
	            	if (args.length < 1) return reply('marque a su amigos!')
					rate = body.slice(1)
					const ti =['4','9','17','28','34','48','59','62','74','83','97','100','29','94','75','82','41','39']
					const kl = ti[Math.floor(Math.random() * ti.length)]
					client.sendMessage(from, 'Como você é gay: *'+rate+'*\n\nSu Porcentaje de gay : '+ kl+'%', text, { quoted: mek })
					break
                  case 'timer':
				...
				case 'delete':
					...
					if (mentioned.length > 1) {
						teks = 'Comando recibido,mensage eliminado :\n'
				...
				case 'openanime':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isAnime) return reply('modo anime ya está activo')
						anime.push(from)
						fs.writeFileSync('./src/anime.json', JSON.stringify(anime))
						reply('Activado con sucesso modo anime en este grupo ✔️')
					} else if (Number(args[0]) === 0) {
						anime.splice(from, 1)
						fs.writeFileSync('./src/anime.json', JSON.stringify(anime))
						reply('Modo anime desactivado con sucesso en este grupo ✔️')
					} else {
				...
				case 'nsfwtrap':
					...
					break
				case 'animekiss':
                if (!isAnime) return reply(' *Deve activar el modo Anime* ')
					anp = getRandom('.gif')
					rano = getRandom('.webp')
				...
				case 'setfoto':
                    ...
                    reply('Icono del Grupo cambiado con exito')
                    break			
                case 'level':
                ...
                case 'leveling':
					...
					if (args[0] === 'on') {
                    if (isLevelingOn) return reply('*El comando de level ya estaba ativo*')
				...
				break 
					...
					if (Number(args[0]) === 1) {
						...
						reply('Grupo anti-link activado con sucesso en este grupo ✔️')
						...
						if (!isantilink) return reply('modo de grupo anti-link fue desabilitado ')
                ...
                roman = `${body.slice(8)}`
                     if (args.length < 1) return reply('Codigo o texto, mano??')
                     if (args.length > 10) return reply('menos de 10 caracteres')
			    ...
			    case 'lovemake':
              	    if (args.length < 1) return reply('Donde esta el texto, hermano??')
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'Texto largo,un maximo de 10 caracteres', text, {quoted: mek})
                    buff = await getBuffer(`https://api.vhtear.com/lovemessagetext?text=${teks}&apikey=ANTIGRATISNIHANJENKKK`, {method: 'get'})
                    client.sendMessage(from, buff, image, {quoted: mek, caption: `${teks}`})
			     	break
                	case 'tomp3':
                	client.updatePresence(from, Presence.composing) 
					if (!isQuotedVideo) return reply('Marque un video pfv')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ Fallo al convertir el vídeo para mp3 ❌')
			    ...
			    case 'infonomor':
			        ...
			        break
			    case 'igstalk':
					if (args.length < 1) return reply('ingrese su nombre de usuario!!')
					ige = body.slice(9)
					reply(mess.wait)
				...
				//lgiproses
				case 'tesss':
					if (args.length < 1) return reply('usted quien es,tio?')
					teks = body.slice(7)
					if (teks.length > 8) return reply('Texto largo, con hasta 8 caracteres')
					reply(mess.wait)
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS KIIL-BOT 👑* 」\n\`\`\``
					no = 0
					...
					break
				case 'desligar':
				if (!isOwner) return reply('Comando solo para mi dueño,quien es usted?')
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						const media = await client.downloadAndSaveMediaMessage(encmedia)
						client.sendMessage(from, 'BOT DESACTIVADO CON SUCESSO', text, { quoted: mek })
						ran = getRandom('.webp')
						await ffmpeg(`./${media}`)
							.input(media)
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`MANDE EL TITULO DE UNA FOTO SI DESEA APAGAR EL BOT!`)
					}
					break
			case 'closegc':
					client.updatePresence(from, Presence.composing) 
					...
					var nomor = mek.participant
					const close = {
					text: `Grupo cerrado por el administrador @${nomor.split("@s.whatsapp.net")[0]}\*solo los administradores* pueden enviar mensajes`,
					contextInfo: { mentionedJid: [nomor] }
					}
...
// recursos anime, random @s domina vida
              ...
              case 'cerpen':
               ...
               case 'puisiimg':
                ...
                case 'bukagc':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					open = {
					text: `Grupo abierto por el administrador @${sender.split("@")[0]}\ahora *todos los participantes* pueden enviar mensajes`,
					contextInfo: { mentionedJid: [sender] }
					}
			...
			break
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'setnome':
                   ...
                   client.sendMessage(from, 'Sucesso, se cambio el nombre del grupo', text, {quoted: mek})
                   break
				case 'galaxtext':
					if (args.length < 1) return reply('que quieres, tio')
					teks = body.slice(12)
                ...
                case 'tahta':
					if (args.length < 1) return reply(mess.blank)
					teks = body.slice(7)
					if (teks.length > 9) return reply('texto largo, hasta 9 caracteres')
					reply(mess.wait)
					buffer = await getBuffer(`https://api.vhtear.com/hartatahta?text=${teks}&apikey=ANTIGRATISNIHANJENKKK`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Harta Tahta '+teks})
					break
				...
				case 'party':
					if (args.length < 1) return reply(mess.blank)
					part = body.slice(7)
					if (part.length > 20) return reply('Texto largo,hasta 20 caracteres')
					reply(mess.wait)
					buffer = await getBuffer(`https://api.vhtear.com/partytext?text=${part}&apikey=ANTIGRATISNIHANJENKKK`)
				...
				case 'rtext':
					if (args.length < 1) return reply(mess.blank)
					tels5 = body.slice(7)
					if (tels5.length > 10) return reply('Texto largo, com hasta 10 caracteres')
					reply(mess.wait)
					buffer = await getBuffer(`https://api.vhtear.com/romancetext?text=${tels5}&apikey=ANTIGRATISNIHANJENKKK`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: tels5})
					break
				case 'water':
					if (args.length < 1) return reply(mess.blank)
					tels = body.slice(7)
					if (tels.length > 15) return reply('El texto es muy largo,hasta 20 caracteres')
					reply(mess.wait)
					anu = await fetchJson(`https://zeksapi.herokuapp.com/api/tfire?text=${tels}&apikey=xptnbot352`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek})
          ...
          reply('Espere..')
				...
				if (!isGroup) return reply(mess.only.group)
					...
					break
			    case 'owner':
                    client.sendMessage(from, {displayname: "JuanCarlos", vcard: vcard}, MessageType.contact, { quoted: mek})
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre en contacto con mi propietario para adquirir o acesso Premium!' ,text, { quoted: mek })
					...
					case 'addvip':  
					if (!isOwner) return reply(mess.only.ownerB)
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato con mi propiertario para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					break	
					case 'cekvip': 
					if (!isPremium) return reply('Usted no es un Miembro Premium, entre en contacto con mi proprietário para adquirir o acesso Premium!' ,text, { quoted: mek })
					me = client.user
					uptime = process.uptime()
					client.sendMessage(from,  `*──────────────────*\n*Nombre do bot:* KILL-BOT\n*─────────────────*\n『 *𝐕𝐈𝐏 𝐔𝐒𝐄𝐑*』\n*──────────────────*\n*•Número:* *${sender.split("@s.whatsapp.net")[0]}*\n*•Status:* *ATIVO*\n*──────────────────*\n*Status Bot:* *${kyun(uptime)}*\n\n*USTED ES UN MIEMBRO PREMIUM* 🐊🚩\n*──────────────────*` , text, { quoted: mek, })
					break
					case 'dellvip':
					if (!isOwner) return reply(mess.only.ownerB)
					if (!isPremium) return reply('Usted no es un Miembro Premium, entre em contato con mi proprietário para adquirir o acesso Premium!' ,text, { quoted: mek })
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT ✨* 」\n*│+ wa.me/5219212830538*╰──────*「 *KILL* 」*────*\n\n*_SE QUER SER ADMIN DO BOT_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
                ...
                break
                    case 'modbotlist':
					teks = 'Esta lista es de usuários premium :\n'
					for (let p of mod) {
						teks += `~> @${p.split('@')[0]}\n`
					}
				...
				case 'loli1':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'hmm,quiere ver loli?'})
					break
				case 'hentai':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/8U9GwX4.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Dude beats 2d 😂'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/7VL9cFf.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Buenos dias para todos❤️'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/JaO3yoV.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Buenas tardes,putos😎👍'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do KILL:*\n\n https://youtube.com/channel/'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* JuanCarlos\n*YOUTUBE:* https://youtube.com/channelg\n*WPP:* wa.me/+5219212830538\n*INSTA:* @\n\n\nEspero que te haya gustado el bot 🐊🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'pronomeneu':
					...
					break
				case 'yt2mp3':
					if (args.length < 1) return reply('Donde está el url, hum?')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/yta?url=${args[0]}&apiKey=${apiKey}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					...
					break
				case 'tiktok':
					if (args.length < 1) return reply('Donde está o url, hum?')
				...
				case 'tiktokstalk':
					try {
						...
						console.log(`Error :`, color(e,'red'))
						reply('Posible nombre de usuário inválido')
					}
					break
				case 'nulis':
				case 'tulis':
			...
			reply(mess.wait)
				...
				case 'url2img':
					...
					if (args.length < 2) return reply('Codigo o url, hum?')
				...
				case 'tstiker':
				case 'tsticker':
					if (args.length < 1) return reply('Donde está o texto, hum?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
        ...
        case 'promover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Ok, jefe este chico aqui: @${mentioned[0].split('@')[0]} ahora es admin del grupo!`, mentioned, true)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('usted quiere adicionar a un genio?')
					if (args[0].startsWith('08')) return reply('Use código de país, man')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('Fallo al adicionar al contacto,talvez porque es privado')
					}
					break
				case 'banir':
					if (!isGroup) return reply(mess.only.group)
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Joto removido con exito ajajajaja :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Joto removido con exito ajajaja  : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'admins':
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista de admins d grupo *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
					for (let admon of groupAdmins) {
						no += 1
				...
				case 'bemvindo':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('ya esta ativo.')
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Recurso activado con exito en este grupo😉️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Se desactivo con exito el recurso en este grupo 😡️')
					} else {
		        ...
		        case 'setfoto2':
                    ...
                    break
                case 'bc2':
					if (!isadminbot) return reply('Quien eres jajaja?')
					if (args.length < 1) return reply('.......')
					anu = await client.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
