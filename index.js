	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hola @${num.split('@')[0]}\nBienvenido al grupo *${mdata.subject}*\n\nEspero y te guste el tiempo que estes aqui❤️`
				...
				teks = `Adios @${num.split('@')[0]} ten una buena tarde😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("bot puto")){
		...
		if (messagesC.includes("callate bot")){
			client.updatePresence(from, Presence.composing)
			reply("kyc mierda")
	}
          ...
          case 'snack':
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'modoanime':
					...
					if (args.length < 1) return reply('Hmmmm')
					if ((args[0]) === 'on') {
						if (isAntiRacismo) return reply('modo antiracismo ya está ativo')
    ...
    break
				...
				case 'bloquear':
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(from, `ordenes recibidas,miembro bloqueado ${body.slice(7)}@c.us`, text)
					break
				case 'premiumlist':
					teks = '╭────*「 *PREMIUM USER👑* 」\n'
               ...
               case 'pornhub':
			   reply(mess.wait)
              	    if (args.length < 1) return reply('Code o texto, mano?')
                    teks = body.slice(9)
		...
		case 'bpfont':
           ...
           case 'playmp3':
               ...
               if (anu.error) return reply(anu.error)
                ...
                break 
            case 'smule':
					if (args.length < 1) return reply('Code o url mano?')
					...
					break  
				case 'ttp':
					if (args.length < 1) return reply('Code o texto, hum?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
			...
			break
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'thunder':
					...
					break
                case 'stiltext':
                      if (args.length < 1) return reply('Code o texto?')
                      gh = body.slice(11)
                      gl1 = gh.split("|")[0];
                      gl2 = gh.split("|")[1];
                      buff = await getBuffer(`https://api.vhtear.com/silktext?text=${gl1}&text2=${gl2}&apikey=ANTIGRATISNIHANJENKKK`)
           ...
           case 'shitlist':
			...
			reply(mess.wait)
				...
				case 'url2img':
					...
					if (args.length < 2) return reply('Code o url, hum?')
